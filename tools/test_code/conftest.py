...
def _resolve_project_root() -> Path:
    """conftest 위치와 무관하게 프로젝트 루트를 탐색해 반환."""
    current = Path(__file__).resolve().parent
    for candidate in [current, *current.parents]:
        if (candidate / "pyproject.toml").is_file():
            return candidate
    # 폴백: 기존 테스트 구조(DataTool_dev_code/test_code) 기준
...
def _resolve_datatool_dev_dir() -> Path:
    """현재 워크스페이스 구조에 맞는 개발 코드 폴더 경로를 반환."""
    candidates = [
        PROJECT_ROOT / "DataTool_dev_code",
        PROJECT_ROOT / "DataTool_dev",
    ]
    for candidate in candidates:
        if (candidate / "DataTool_optRCD_proto_.py").is_file():
            return candidate
    # 폴더명이 변경됐더라도 테스트 파일 경로 기준으로 동작하도록 기본값 유지
    return PROJECT_ROOT / "DataTool_dev_code"


DATATOOL_DEV = _resolve_datatool_dev_dir()
