...
- 비선형 연산자 근사 대리 모델. Branch/Trunk 이중 구조로 함수→함수 매핑.
- 입력: Branch는 방전/충전 프로파일 통계치(DTDF), Trunk는 온도/시간.
- PINN과 결합 시 물리 제약 통합으로 외삽 성능 확보.

> 심화 내용(아키텍처 상세, Function Space Mapping, Koopman 연산자, DTDF 정의 등)은 [[Knowledge_PINN_DeepONet]] 참조.
