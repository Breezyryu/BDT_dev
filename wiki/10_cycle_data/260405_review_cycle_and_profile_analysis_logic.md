...
> **작성일**: 2026-04-05
> **최종 갱신**: 2026-04-21 — 260327 파이프라인 상세 병합
> **대상 파일**: `DataTool_dev/DataTool_optRCD_proto_.py`
> **범위**: Tab 1(사이클데이터) — 사이클 분석 + 프로필 분석 전체 파이프라인
>
> 📎 2026-04-21: `260327_review_cycle_profile_data_pipeline` 병합 (경로 그룹 체계, CH 제어 3단 계층, 통합 가능성 분석).
...
8. [파이프라인 상세 (260327 이전 버전)](#8-파이프라인-상세-260327-이전-버전)
...
## 8. 파이프라인 상세 (260327 이전 버전)

> 병합 배경: 2026-03-27 작성된 `260327_review_cycle_profile_data_pipeline`의 고유 내용을 보존. 경로 그룹 3단 계층 체계, CH 제어 3단 팝업, 사이클/프로필 통합 가능성 분석, 성능 병목 분해, 메모리 생명주기 등 이 문서(260405)에 포함되지 않은 관점을 제공한다.

### 8.1 사이클러별 데이터 파일 비교 — PNE vs Toyo
...
#### Step / Rate Confirm (2×3)

```
┌────────────────┬────────────────┬────────────────┐
│  ax1: Time-Vol │ ax2: Time-Crate│  ax3: Time-SOC │
├────────────────┼────────────────┼────────────────┤
│  ax4: Time-Temp│ ax5: (미사용)   │ ax6: (미사용)   │
└────────────────┴────────────────┴────────────────┘
```
...
```
┌────────────────┬────────────────┬────────────────┐
│  ax1: SOC-Vol  │ ax2: Vol-dQ/dV │  ax3: SOC-Vol  │
│  (충전곡선)     │ (미분용량)      │  (2nd)         │
├────────────────┼────────────────┼────────────────┤
│  ax4: SOC-dV/dQ│ ax5: SOC-Crate │ ax6: SOC-Temp  │
└────────────────┴────────────────┴────────────────┘
```
...
```
┌────────────────┬────────────────┬────────────────┐
│  ax1: DOD-Vol  │ ax2: dQ/dV-Vol │ ax3: DOD-Vol   │
├────────────────┼────────────────┼────────────────┤
│  ax4: DOD-dV/dQ│ ax5: DOD-Crate │ ax6: DOD-Temp  │
└────────────────┴────────────────┴────────────────┘
```
...
│  ax1: Time-Vol │ ax2: Time-Crate│  ax3: Time-SOC │
├────────────────┼────────────────┼────────────────┤
│ ax4: Time-V+OCV│ax5:SOC-OCV/CCV │ ax6: Time-Temp │
└────────────────┴────────────────┴────────────────┘
```
...
```
┌────────────────┬────────────────┐
│  ax1: SOC-OCV  │  ax3: Volt-SOC │
│  SOC-rOCV      │                │
│  SOC-CCV       │                │
├────────────────┼────────────────┤
│  ax2: SOC-DCIR │  ax4: OCV-DCIR │
│  (0.1s/1s/10s  │                │
│   /20s/RSS)    │                │
└────────────────┴────────────────┘
```

### 8.6 PNE vs Toyo 프로필 로딩 함수 매핑 (레거시)
...
> 이 레거시 함수들은 `unified_profile_core()` (260406 리빌딩)로 통합되었으나, UI 시그널-슬롯 하위 호환을 위해 병존 중. 점진적 마이그레이션 예정.

### 8.7 dQ/dV 계산 로직 (Chg/Dchg 프로필)
...
### 8.10 PNE SaveEndData.csv 원본 컬럼 인덱스 (상세)
...
1. `plt.close()` 누락 — 탭 리셋 시 열린 figure 미닫음
2. `self.df` 덮어쓰기 — 이전 데이터 GC 전 새 할당 시 피크 메모리 2배
3. 대용량 프로필 — AllProfile 모드에서 채널×사이클 전체 DataFrame 동시 보유

### 8.13 성능 병목 분해 (예상 비율)

#### 사이클 분석 (채널 30개 × 400cyc)
...
### 8.14 Cycle ↔ Profile 파이프라인 통합 가능성 (Phase 진행 중)

#### 통합 가능한 부분

| 영역 | 통합 가능 | 상세 |
|------|:--:|------|
| **경로 파싱** | ✅ | `_parse_cycle_input()`의 CycleGroup 체계를 Profile에서도 사용 가능 |
| **채널 스캔** | ✅ | `os.scandir → _is_channel_folder` 패턴 공통화 |
| **PNE/Toyo 판별** | ✅ | 이미 `check_cycler()` 공용 사용 중 |
| **병렬 로딩 프레임** | ⚠️ 부분 | Cycle(채널) vs Profile(채널×사이클) 단위 차이 |
| **인덱스 매핑** | ✅ | `flat_idx_of` + `subfolder_map` 체계 Profile 도입 |
| **탭 완성** | ⚠️ 부분 | Cycle 전용 분류정보 바로 완전 통합은 어려움 |
| **CH 제어** | ✅ | 이미 공유 사용 중 |
| **범례 처리** | ✅ | 그라데이션 로직 공통 적용 |

#### 통합 불가능한 부분

| 영역 | 이유 |
|------|------|
| **데이터 로딩 함수** | Cycle(요약 CSV)과 Profile(시계열)이 읽는 데이터 자체가 다름 |
| **6축 구성** | 사이클: 고정 6축, 프로필: 타입별 축 구성 다름 |
| **사이클 분류** | Cycle 전용 (Profile에는 불필요) |
...
Phase 2: 채널 스캔 + 인덱스 통일 (영향 범위: 중간)
  └── flat_idx_of + subfolder_map 공통 유틸리티

Phase 3: 탭 생성 기반 통합 (영향 범위: 중간)
  └── _finalize_base_tab() 공통 기반 + 확장 패턴
```

> **현황 (260406 이후)**: `unified_profile_core()` 6-Stage 파이프라인으로 Profile 내부는 통합 완료. Cycle↔Profile cycle_map 공유까지 완료. 경로 파싱 통일(Phase 1)은 점진적 전환 중.
