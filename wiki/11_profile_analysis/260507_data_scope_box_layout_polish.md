...
이음 :  [ 이어서 ][  분리  ][  루프  ]    ☑ CV포함
X축  :  [  SOC  ][  DOD  ][  시간  ]    ☐ 방전→충전
   ↑ 라벨 X 통일      ↑ 80px 균일      ↑ 체크박스 X 통일

→ 라벨/첫 버튼/체크박스 모두 같은 X 좌표 (3중 상하 정렬)
```
...
### 7차 — 체크박스 라디오 그룹 우측 1열 정렬

| 행 | 구성 | 위젯 합계 |
|---|---|---|
| 행 1 | 프리셋: [(선택)▾] | 48+4+100 ≈ **152px** |
| 행 2 | 구간: [사이클\|충전\|방전]   ☐ Rest포함 | 48+4+244+12+90 ≈ **398px** |
| 행 3 | 이음: [이어서\|분리\|루프]   ☑ CV포함 | 48+4+244+12+80 ≈ **388px** |
| 행 4 | X축: [SOC\|DOD\|시간]   ☐ 방전→충전 | 48+4+244+12+100 ≈ **408px** |

체크박스 3개를 행 1 에서 분리해 각 라디오 그룹 우측에 1열 정렬:
- Rest포함 → 구간 우측 (행 2)
- CV포함 → 이음 우측 (행 3)
- 방전→충전 → X축 우측 (행 4)

라디오 그룹 폭 244px 동일 + GROUP_GAP 12 동일 → **체크박스 시작 X 좌표 통일**
(행 2/3/4 의 ☐ 박스가 같은 세로선에 정렬). 행 1 (프리셋) 은 콤보만 단독.
