---
title: "Origin vs optRCD_proto 매칭 비교 (결과 차이 병합)"
tags: [bdt-history, origin-vs-optrcd, comparison]
updated: 2026-04-21
---

# Origin vs optRCD_proto 매칭 비교

> 📎 2026-04-21: `origin_vs_optRCD_result_diff_260211.md`(결과 차이 3건)를 본 파일 말미 "결과 차이 (260211)" 섹션으로 병합.
...
> 원본: `origin_vs_optRCD_result_diff_260211.md` — 2026-02-11 작성. `BatteryDataTool_origin` (14,168줄) vs `BatteryDataTool_optRCD.py` (15,275줄) 비교 시점. 3건 모두 **Origin의 버그가 optRCD에서 수정된 것**으로, 의도치 않은 결과 변경은 없다.

### 차이 1. `smoothdegree` float → int 변환 (버그 수정, 영향도 낮음)

**영향 함수**: `pne_chg_Profile_data` / `pne_chg_Profile_batch`, `pne_dchg_Profile_data` / `pne_dchg_Profile_batch`
**발생 조건**: UI `smoothdegree = 0` 설정 시 자동 계산 로직 동작.

**Origin** (L1407, L1465):
```python
if smoothdegree == 0:
    smoothdegree = len(df.Profile) / 30   # Python3: float (3.333...)
df.Profile["delvol"] = df.Profile["Voltage[V]"].diff(periods=smoothdegree)
```
- 최신 pandas에서 `periods` float 전달 시 **TypeError** 가능. 구버전에서는 암묵적 int 변환.

**optRCD individual** (L1856, L1911): `smoothdegree = int(len(df.Profile) / 30)` — 명시적 int, 구버전 pandas와 동일 결과.

**optRCD batch** (L961, L1019):
```python
sd = smoothdegree if smoothdegree != 0 else max(int(len(df.Profile) / 30), 1)
```
- `max(..., 1)` 추가로 **최소 periods=1 보장**.
- Profile 행 < 30 시: Origin/individual은 `diff(periods=0)` → dQdV/dVdQ = NaN, batch는 `diff(periods=1)` → **실제 차분값**.

| 조건 | Origin | optRCD individual | optRCD batch |
|---|---|---|---|
| Profile ≥ 30행 | 동일 | 동일 | 동일 |
| Profile < 30행 | dQdV/dVdQ = NaN/inf | dQdV/dVdQ = NaN/inf | **실제 차분값** |
| pandas 최신 | TypeError 가능 | 정상 | 정상 |

### 차이 2. `self.dvscale` vs `dvscale` 혼용 버그 수정 (영향도 높음)

**영향 함수**: `dchg_confirm_button` 내 dVdQ 그래프 Y축 스케일 설정.
**발생 조건**: 방전 Profile(Dchg) 확인 버튼 클릭 시 **항상**.
...
graph_profile(Dchgtemp[1].Profile.SOC, Dchgtemp[1].Profile.dVdQ, Chg_ax4,
              ...
              "DOD", "dVdQ", temp_lgnd)
```
- `dvscale`은 `Profile_ini_set()` 반환 로컬 변수, `self.dvscale`은 `chg_confirm_button()`에서만 설정.
- 문제: (1) chg 미실행 시 `AttributeError` 크래시, (2) chg 먼저 실행해도 이전 설정값 적용으로 불일치, (3) 같은 호출 내 x_min/y_hlimit/y_gap이 서로 다른 변수 참조.

**optRCD** (L10769):
```python
graph_profile(..., 0, 1.3, 0.1, -5 * dvscale, 0.5 * dvscale, 0.5 * dvscale, ...)
```
- 전부 로컬 `dvscale` 통일.
...
| chg(scale=1) 후 dchg(scale=2) | x_min=-10, y=0.5, gap=0.5 (불일치) | x_min=-10, y=1.0, gap=1.0 (일치) |

### 차이 3. `pne_search_cycle` index_min 빈 경우 처리 (영향도 낮음)

**영향 함수**: PNE 데이터 로딩 전반. **단, batch 함수 사용 시에만 차이** — individual 함수(`pne_rate/chg/dchg_Profile_data`)는 여전히 `pne_data()`→`pne_search_cycle()` 경로 유지 → Origin과 동일 동작.
**발생 조건**: SaveEndData에 요청 사이클의 **이전 사이클**이 없는 엣지 케이스 (예: 사이클 5 요청인데 사이클 4 미기록).

**Origin** `pne_search_cycle` (L906~909):
```python
if len(index_min) != 0:
    ...
    file_end = -1
```
`pne_data` (L874): `if os.path.isdir(rawdir) and (filepos[0] != -1):` — filepos=-1이면 **전체 스킵** → 빈 DataFrame.

**optRCD batch** `_pne_load_profile_raw` (L840~843):
```python
if len(index_min) == 0:
    file_start = 0    # ← 0으로 설정
else:
...
| 변경 내용 | 위치 | 결과 영향 |
|---|---|---|
| ThreadPoolExecutor 병렬 로딩 | 모든 confirm_button | 없음 |
| `_pne_load_profile_raw` SaveData 일괄 로딩 | PNE batch | 없음 (메모리 필터링) |
| `pne_min_cap` / `toyo_min_cap` 1회 호출 | batch | 없음 |
| `check_cycler` cyclefolder당 1회 호출 | confirm_button | 없음 |
| `lru_cache` on `_load_pne_index_files` | PNE 관련 | 없음 |
| `global writer` → 로컬 writer | confirm_button | 없음 |
| `_init_confirm_button` / `_setup_file_writer` 헬퍼 | confirm_button | 없음 |
| `_create_plot_tab` / `_finalize_plot_tab` 헬퍼 | confirm_button | 없음 |
| `_setup_legend` 헬퍼 | confirm_button | 없음 |
| `dQdV/dVdQ/delcap/delvol = 0` 초기화 생략 | PNE chg/dchg batch | 없음 (`.diff()`가 덮어씀) |
| `AllProfile` 모드 추가 | chg/dchg/rate/continue | **새 기능** (기존 동작 영향 없음) |

### 종합 (3건 모두 버그 수정)

| # | 변경 | 분류 | 실 영향도 |
|---|---|---|---|
| 1 | `smoothdegree` int 변환 | 버그 수정 | 낮음 (Profile < 30행만) |
